...
export const confirmEmail = async (token: string, req: Request): Promise<ResponseSuccess> => {
  ...
  // If not such user exists, or their token is expired, bail.
  ...
  if (user.emailStatus === 'unconfirmed') {
    ...
    throw new Error(
      `Consistency violation: User ${user.id} has a email proof token, but somehow alse has an emailStatus of "${user.emailStatus}"! (This should never happen)`,
    );
  }
};
